package timus.task_1018;

/*
Представьте, как должна выглядеть яблоня в двоичном компьютерном мире.
Вы правы, она выглядит как двоичное дерево, то есть любая ветка двоичной яблони может ветвиться в точности на две новые
ветки.
Занумеруем целыми числами корень двоичной яблони, точки ветвления и верхушки верхних ветвей.
Пусть корень дерева всегда имеет номер 1, а все числа, используемые при нумерации, лежат в пределах от 1 до N, где N —
 общее число пронумерованных точек.
...
Вот почему некоторые ветви следует удалить из дерева. Но вы также заинтересованы в удалении ветвей, которое приведёт к
 минимальной потере яблок.
...
Исходные данные
В первой строке даны числа N и Q (2 ≤ N ≤ 100; 1 ≤ Q ≤ N − 1). N обозначает количество пронумерованных точек в дереве,
а Q — количество ветвей, которые нужно сохранить.
Следующие N − 1 строк содержат описание ветвей. Описание каждой ветви состоит из трёх чисел, разделённых пробелом.
Первые два из них определяют конечные точки ветви, третье — число яблок на данной ветви. Вы можете считать, что любая
ветвь содержит не более 30000 яблок.
Результат
Выведите единственное число — максимальное количество яблок, которое можно сохранить. Не забудьте сохранить корень
яблони ;-)
 */
